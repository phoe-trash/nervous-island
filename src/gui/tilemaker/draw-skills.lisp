...
(defmethod draw-skills (state (attack na:attack) &rest attacks)
  (flet ((preprocess (attacks)
    ...
    (let* ((attacks (preprocess (cons attack attacks)))
           ...
           (hq-tile-p (typep (tile state) 'nt:hq))
           (allocated-left-p (and (not hq-tile-p)
                                  (allocated-left-p state direction)))
           (allocated-right-p (and (not hq-tile-p)
                                   (allocated-right-p state direction)))
