...
(define-test space-make-spaces
  (let* ((things (list (nc:axial 0 0)
    ...
    (true (typep spaces 'dict))
    (is = 2 (dict-count spaces))
    (dolist (axial axials)
      (let ((thing (dict-find spaces axial)))
  ...
  (let* ((axial (nc:axial 0 0))
         ...
         (spaces (nsp:make-spaces space-1 space-2 space-3 axial)))
    (is = 1 (dict-count spaces))))
  ...
  (let* ((token (make-instance 'space-test-token))
    ...
    (flet ((make (q r &rest args)
      ...
      (let* ((space-1 (make 0 0 :tokens (list token)))
             ...
             (spaces (nsp:make-spaces space-1 space-2 space-3 space-4)))
        (is eqv space-1 (nsp:find-element spaces token))
        (is eqv space-2 (nsp:find-element spaces instant))
        (is eqv space-3 (nsp:find-element spaces unit))
        (is eqv space-4 (nsp:find-element spaces foundation))
        (let ((other-unit (make-instance 'space-test-other-unit)))
          (false (nsp:find-element spaces other-unit))))
      (let* ((omega-space (make 0 0 :tokens (list token) :overlay instant
                                    :unit unit :unit-rotation 0
                                    :foundation foundation))
             (spaces (nsp:make-spaces omega-space)))
        (is eqv omega-space (nsp:find-element spaces token))
        (is eqv omega-space (nsp:find-element spaces instant))
        (is eqv omega-space (nsp:find-element spaces unit))
        (is eqv omega-space (nsp:find-element spaces foundation))
        (let ((other-unit (make-instance 'space-test-other-unit)))
          (false (nsp:find-element spaces other-unit)))))))
